...
const MindMapViewerContentInner: React.FC<MindMapViewerProps> = ({ mindmapId, onClose }) => {
  // Brainstorm Chat modal state
  const [showBrainstormChat, setShowBrainstormChat] = useState(false);
  const [unreadChatCount, setUnreadChatCount] = useState(0);
  // Use mindmapId from props instead of route params
  ...
  // No-op reference to avoid unused warning in case throttledBroadcastLiveChange not directly invoked yet
  void throttledBroadcastLiveChange;
  ...
  const fetchMindMapFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      // Fetch mindmap by ID directly since we're in inline viewer
      const { data: mindmapsData, error: mindmapsError } = await supabase
        .from("mindmaps")
        .select("*")
        .eq("id", MapId);

      if (mindmapsError || !mindmapsData || mindmapsData.length === 0) {
        setLoadError("Mindmap not found.");
        setIsLoading(false);
        return;
      }

      const map = mindmapsData[0];
      ...
      if (!isCreator && !isCollaborator) {
        console.log('[MindMap] No permission to edit this mindmap.');
        showToast('You do not have permission to edit this mindmap', 'error')
        onClose()
        return;
      } const processedMap = {
        id: map.id,
      ...
      const rawDrawingData = map.drawing_data
        ? decompressDrawingData(map.drawing_data)
        : { strokes: [] };
      const drawingDataToSet: DrawingData = rawDrawingData && (rawDrawingData as any).strokes
        ? rawDrawingData as DrawingData
        : { strokes: [] };
  ...
  }, [MapId, isLoggedIn, maps, collaborationMaps, currentMap, showToast, onClose]);
  ...
  }, [currentMap?.id, MapId, fetchMindMapFromSupabase, validateAndFixEdgeIds])
  ...
  const handleViewNavigation = () => {
    // View navigation not applicable in inline viewer
    // User can simply close the viewer to return to notes
  }
  ...
  // Prepare error screen (rendered conditionally in main return to keep hook order stable)
  const errorScreen = loadError && (
  ...
  return (
    <>
      ...
      <div
        className={`w-full h-full relative mindmap-content ${isLoading ? 'loading' : ''}`}
        style={{
          '--mindmap-font-family': (() => {
      ...
      >
        ...
        <div
          ...
          className={`w-full h-full relative ${isFullscreen ? '' : ''}`}
          style={{
            zIndex: 1,
            ...
            onSave={async (details) => {
              try {
                ...
                if (isPermalinkChanged) {
                  ...
                  // In inline viewer, refresh the component instead of navigating
...
const MindMapViewer: React.FC<MindMapViewerProps> = (props) => {
  return (
    <ReactFlowProvider>
      <MindMapViewerContentInner {...props} />
    </ReactFlowProvider>
  )
}
